...
module.exports = router;-codes/products:
 *   get:
 *     summary: Get available QR code products
 *     tags: [QR Codes]
 *     parameters:
 *       - in: query
 *         name: category
 ...
 *     responses:
 *       200:
 *         description: QR products retrieved successfully
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                       type: array
 *                       items:
 *                         $ref: '#/components/schemas/QRProduct'
 *                     categories:
 *                       type: array
 *                       items:
 *                         type: string
 ...
 */
router.get('/products', [
  queryValidator('category').optional().isString().trim(),
  queryValidator('minPrice').optional().isFloat({ min: 0 }),
  queryValidator('maxPrice').optional().isFloat({ min: 0 }),
  queryValidator('sortBy').optional().isIn(['price_asc', 'price_desc', 'name_asc', 'name_desc', 'created_desc']),
  ...
  if (category) {
    whereConditions.push(`design_category = $${paramIndex}`);
  ...
  // Get total count
  const countQuery = `SELECT COUNT(*) FROM qr_products WHERE ${whereConditions.join(' AND ')}`;
  const countResult = await query(countQuery, queryParams);
  const totalCount = parseInt(countResult.rows[0].count);

  // Get products
  const productsQuery = `
    SELECT * FROM qr_products 
  ...
  const categoriesResult = await query(
    'SELECT DISTINCT design_category FROM qr_products WHERE is_active = true AND design_category IS NOT NULL ORDER BY design_category'
  );
  ...
  res.json({
    ...
    data: {
      ...
      categories: categoriesResult.rows.map(row => row.design_category),
      pagination: {
        page: parseInt(page),
...
/**
 * @swagger
 * /api/qr-codes/products/{id}:
 *   get:
 *     summary: Get QR product by ID
 *     tags: [QR Codes]
 *     parameters:
 *       - in: path
 *         name: id
 ...
 *     responses:
 *       200:
 *         description: QR product retrieved successfully
 *       404:
...
router.get('/products/:id', [
  ...
  const result = await query(
    'SELECT * FROM qr_products WHERE id = $1 AND is_active = true',
    [id]
  );

  if (result.rows.length === 0) {
    throw createError('QR product not found', 404, 'PRODUCT_NOT_FOUND');
  }
