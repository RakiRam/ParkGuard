...
router.post('/initiate-call', [
  ...
  await transaction(async (client) => {
    ...
    let callResult = { callSid: 'mocked_failed', status: 'failed', estimatedConnectTime: 0 };
    try {
      callResult = await initiateVoipCall({
        callerPhone,
      ...
      logger.warn({ err, vehicleId }, 'Twilio VoIP call initialization failed. System isolated the error to prevent crash.');
    }
...
router.post('/send-message', [
  ...
  await transaction(async (client) => {
    ...
    let smsResult = { sid: 'mocked_failed', status: 'failed' };
    try {
      ...
      logger.warn({ err, vehicleId }, 'Twilio SMS failed. System isolated the error to prevent crash.');
    }
